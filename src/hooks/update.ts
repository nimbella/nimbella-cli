...
const debug = require('debug')('nimbella:brewhook')
...
export const brewHook: Hook<'update'> = async function() {
  ...
  let binPath
  try {
    binPath = fs.realpathSync(path.join(brewRoot, 'bin/nim'))
  } catch (error) {
  ...
  if (binPath && binPath.startsWith(path.join(brewRoot, 'Cellar'))) {
    cellarPath = path.resolve(binPath, path.dirname(path.relative(binPath, path.join(brewRoot, 'Cellar/nimbella'))))
  }
